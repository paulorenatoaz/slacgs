...
if ! git diff --quiet -- .gitignore coinfosim/publish .github/workflows scripts/publish_output_to_pages.sh 2>/dev/null; then
  git add -A
...
if ! git ls-remote --exit-code --heads "$REMOTE" "$PAGES" >/dev/null 2>&1; then
  ...
  touch .nojekyll
  echo "<!doctype html><title>coinfosim</title><h1>Publishing…</h1>" > index.html
  git add -A
