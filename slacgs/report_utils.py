...
from slacgs import report_service_conf
...
def create_loss_vs_n_graphs(scenario, scenario_id, alpha_index, alpha_index_text):
	...
	for loss_type in loss_types:
		...
		for sim in scenario:
			...
			n_values = np.log2(sim['model_tag']['N'])
...
def create_loss_vs_alpha_graphs(scenario, scenario_id, alpha_index, alpha_index_text):
    ...
    for loss_type in loss_types:
        ...
        n_indexes_to_plot = [n_values.index(n) for n in n_values_to_plot]
...
def create_html_scenario_report(scenario, scenario_id):

	output_path = os.path.join(report_service_conf['reports_path'], 'scenario_' + str(scenario_id) + '_report.html')
...
def create_scenario_report(scenario_params, scenario_id):
	...
	create_html_scenario_report(scenario_data, scenario_id)
