...
from coinfosim.config import load_config, get_output_dir, get_reports_dir, get_data_dir

# Legacy imports for Google Drive/Sheets functionality (optional)
# These are only needed for deprecated features - install with: pip install coinfosim[legacy]
try:
...
report_service_conf: dict
    ...
    - 'data_path': str, the path where JSON data files are stored
    - 'visualizations_path': str, the path for visualization images
    - 'graphs_path': str, the path for graph images  
    - 'tables_path': str, the path for CSV tables
    - 'reports_path': str, the path where HTML reports are stored
    - 'user_email': str or None, the user's email used for Google services authentication (legacy)
    - 'drive_service': object or None, the Google Drive service object (legacy)
    ...
    This dictionary is automatically initialized from coinfosim.config system.
    Call init_report_service_conf(config) to update paths.
"""


def init_report_service_conf(config=None):
    """Initialize report_service_conf from centralized config.
    
    Args:
        config: Configuration dictionary (loads default if None)
        
    Returns:
        dict: Initialized report service configuration
    """
    if config is None:
        config = load_config()
    
    # Get paths from centralized config with auto-creation
    output_path = get_output_dir(config, create=True)
    reports_path = get_reports_dir(config, create=True)
    data_path = get_data_dir(config, create=True)
    
    # Build paths for subdirectories (images stored under reports, not root output)
    visualizations_path = reports_path / 'images' / 'visualizations'
    graphs_path = reports_path / 'images' / 'graphs'
    tables_path = data_path / 'tables'
    
    # Create subdirectories
    for path in [visualizations_path, graphs_path, tables_path]:
        path.mkdir(parents=True, exist_ok=True)
    
    return {
        'output_path': str(output_path),
        'data_path': str(data_path),
        'visualizations_path': str(visualizations_path),
        'graphs_path': str(graphs_path),
        'tables_path': str(tables_path),
        'reports_path': str(reports_path),
        # Legacy Google services fields
        'user_email': None,
        'drive_service': None,
...
# Initialize with default config at module load
report_service_conf = init_report_service_conf()


def is_param_in_simulation_reports(params, test_mode=False):
  """Check if a parameter is already in the simulation_reports.json file.

  Parameters:
      params (list): The parameter to be checked.
      test_mode (bool): Whether to check test mode reports. Defaults to False.
  ...
  json_path = os.path.join(report_service_conf['data_path'], json_filename)
...
def set_report_service_conf(path_to_google_cloud_service_account_api_key=None, user_google_account_email=None,
  ...
  Parameters:
    coinfosim_password (str): The password used to enable our Report Service. Defaults to None.
  ...
  if coinfosim_password is not None and not isinstance(coinfosim_password, str):
  ...
  if path_to_google_cloud_service_account_api_key is not None:
    ...
    key_obj = eval(get_key(coinfosim_password))
