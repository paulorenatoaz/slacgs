"""
Shared HTML rendering helpers for CoSenSim reports.
...
Example:
    >>> from cosensim.reporting.html_render import render_section, render_matrix_table
...
# Shared CSS used by every CoSenSim report. Inspired by the Sensor Crossover
...
def format_param_label(param_index: int, dim: int) -> dict:
    ...
    CoSenSim parameter vectors layout (for ``dim`` features) is
...
def render_page(
    ...
    footer_text = footer or (
        "Generated by CoSenSim — Loss Analysis of Linear Classifiers on Gaussian Samples."
    )
