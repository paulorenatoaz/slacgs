...
class Simulator:
  ...
  """A simulator for Linear classifier Loss analysis in order to evaluate Trade Off Between Samples and Features in Classification Problems."""

  def __init__(self, model, dims=[1,2,3], loss_types = ['EMPIRICAL_TRAIN', 'THEORETICAL', 'EMPIRICAL_TEST'], test_samples_amt=1024, step_size=5, max_steps=200, first_step=100, precision=1e-6, augmentation_until_n = 2**10):

    """
    :param model (Model): Linear Classifier Loss Analysis Model
  ...
  def print_N_progress(self, n, max_iter, iter_per_step,fig):
  ...
  def plot_compare(self, dims: (), fig):
  ...
  def intersect_elip_and_origin_line(self, dim):
  ...
  def loss_bayes_empirical(self,clf, data_train):
    ...
    return 1 - np.mean([class_probs[0:int(len(class_probs)/2)].T[1], class_probs[int(len(class_probs)/2):int(len(class_probs))].T[0]])
  ...
  def start(self) :
    ...
    while True:
      for n in N :
        ...
        iter_per_step = math.floor(self.step_size*augmentation_factor) if n < self.augmentation_until_n else self.step_size
