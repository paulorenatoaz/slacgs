...
from slacgs.utils import is_notebook
from slacgs.progress import ProgressTracker
...
class Simulator:
  ...
  def __init__(self, model: Model, dims=None, dims_to_compare=None, loss_types = ('EMPIRICAL_TRAIN', 'THEORETICAL', 'EMPIRICAL_TEST'), test_samples_amt=1024, step_size=5, max_steps=200, min_steps=100, precision=1e-6, augmentation_until_n = 1024, test_mode=False, full_n_range=False, verbose=True, debug=False):
    ...
    self.full_n_range = full_n_range
    ...
    # In test mode, limit max_n to 1024 to prevent continued N growth after convergence
    if test_mode and model.max_n > 1024:
        model.max_n = 1024
    ...
    self.progress = ProgressTracker(verbose=verbose, debug=debug)


  def generate_report_data(self):
    """
    Generate ReportData from current simulator state.
    
    This creates a lightweight data object containing only the information
    Report needs, breaking the circular dependency between Simulator and Report.
    ...
    return ReportData(
        ...
        test_samples_amt=self.test_samples_amt,
        full_n_range=self.full_n_range,
        ...
        N=self.model.N.tolist() if hasattr(self.model.N, 'tolist') else list(self.model.N),
        sigma=self.model.sigma,
        rho=self.model.rho,
        dim=self.model.dim,
        mean_pos=self.model.mean_pos,
        mean_neg=self.model.mean_neg,
        dictionary=self.model.dictionary,
        rho_matrix=self.model.rho_matrix,
        max_n=self.model.max_n,
        cov=self.model.cov.tolist() if hasattr(self.model.cov, 'tolist') else self.model.cov,
    )
  ...
  def run(self):
    ...
    self.progress.log_simulation_init(
        self.model.params, 
        self.dims, 
        self.model.N.tolist() if hasattr(self.model.N, 'tolist') else list(self.model.N),
        test_mode=self.test_mode,
    ...
    # In test mode, limit N to first 10 values to keep test mode fast while still processing up to 1024
    # (Report generation requires minimum cardinalities to build N_star matrices)
    if self.test_mode and not self.full_n_range:
      self.model.N = self.model.N[0:10]
    ...
    while True:
      for n in N :
        ...
        # Log cardinality start to progress tracker
        self.progress.log_cardinality_start(n, self.dims, max_iter)

        # terminal output N progress bar (deprecated - using progress tracker instead)
        # if self.verbose:
        ...
        for i in range(max_iter):
          ...
          if (i+1) % iter_per_step == 0:
            ...
            ## Update progress tracker at checkpoints
            if (i+1) % iter_per_step == 0:
              self.progress.update_checkpoint(i+1, loss_sum, iter_N, self.dims)

            ## print report for each step (deprecated - using progress tracker instead)
            # if self.verbose:
        ...
        # Log cardinality completion to progress tracker
        cardinality_elapsed = time.time() - st
        self.progress.log_cardinality_complete(n, cardinality_elapsed)
        self.progress.update_cardinality_progress(n)
        ...
        ## save visualization for this cardinality to build animated GIF
        self.report.save_visualization_png_image_file()
    ...
    self.progress.log_simulation_complete(
        elapsed_time, 
        self.progress.cardinalities_processed,  # Use actual count from progress tracker
        len(self.dims),
        reports_dir=reports_dir,
        data_dir=data_dir
    )
    self.progress.finish_cardinality_loop()
