"""
Shared HTML rendering helpers for SLACGS reports.
...
Example:
    >>> from slacgs.reporting.html_render import render_section, render_matrix_table
...
# Shared CSS used by every SLACGS report. Inspired by the Sensor Crossover
...
def format_param_label(param_index: int, dim: int) -> dict:
    ...
    SLACGS parameter vectors layout (for ``dim`` features) is
...
def render_page(
    ...
    footer_text = footer or (
        "Generated by SLACGS — Loss Analysis of Linear Classifiers on Gaussian Samples."
    )
