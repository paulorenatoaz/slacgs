...
from slacgs.reporting.html_render import format_param_label
...
def detect_alpha_index(scenario_params, scenario_dim):
	...
	# Special case in 3D: the SLACGS scenario 4 uses a single slider for
...
def create_html_scenario_report(scenario, scenario_id, test_mode=False):
	...
	def _build_html(embed: bool) -> str:
		...
		overview = (
			...
			f"SLACGS simulations sharing a common Gaussian model template; "
		...
		return render_page(
			title=f"SLACGS Scenario Report — Scenario {scenario_id}",
			subtitle=subtitle,
			toc_items=toc,
